...
export function ChatPane({ userId, paneId, lawPrompt, tonePrompt, policyPrompt, onSendMessage, onClearChat, onStreamingStatusChange, registerSendHandler, unregisterSendHandler, registerResetStatesHandler, unregisterResetStatesHandler }: ChatPaneProps) {
  ...
  useEffect(() => {
    if (isStreaming) {
      ...
      // If we're streaming but don't have actual phases from the backend,
      // use a cycle of predefined phases to show progress
      ...
      // Only use the timer if we don't have a real phase from the backend
      if (!phaseTimerRef.current && showLocalPendingIndicator && !hasBackendPhase) {
        // Check the query to determine if we should cycle through all phases or just show "Thinking"  
        // For very short queries like "hello", it's likely using the fast path, so just show "Thinking"
        const latestUserMessage = messages.find(msg => msg.role === "user");
        const isSimpleQuery = latestUserMessage && latestUserMessage.content.length < 10;
        
        // Use only "Thinking" for simple queries, full cycle for complex ones
        const phases = isSimpleQuery 
          ? ["Thinking"]
          : [
              "Thinking",
              "Searching web",
        ...
        const cycleThroughPhases = () => {
          setCurrentProcessingPhase(phases[phaseIndex]);
          console.log(`[ChatPane] Processing phase: ${phases[phaseIndex]}`);
          phaseIndex = (phaseIndex + 1) % phases.length;
          
          // Schedule next phase update - much faster cycling for better visibility
          phaseTimerRef.current = window.setTimeout(cycleThroughPhases, 800);
        };
