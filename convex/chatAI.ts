...
async function initializeAIResponse(
  ...
  const previousMessages = await ctx.runQuery(api.chat.getMessages, {
    userId,
    paneId,
  });
  const formattedHistory = previousMessages.map(
...
function determineSystemPrompts(args: {
...
async function handleNoToolResponseFlow(
  ...
  if (!executor) {
    console.error(
      `[handleNoToolResponseFlow] AgentModeOffExecutor not found. Falling back to direct response.`,
    );
    const modelName = selectedModel || DEFAULT_MODEL_NAME;
    const systemPromptsText = systemPrompts
      ? combineSystemPrompts(systemPrompts)
      : "";
  ...
  try {
    ...
    console.error(
      `[handleNoToolResponseFlow] AgentModeOffExecutor error: ${error}`,
    );
    return "I'm Elixer, your friendly AI assistant. I'm having trouble processing your request right now. How else can I help you?";
  }
}
...
async function callFinalLLMSynthesis(
  ...
  const searchMatch = toolContent.match(
    /<!-- SEARCH_SUGGESTIONS_HTML:([\s\S]*?) -->/i,
  );
  if (searchMatch) {
    searchSuggestionsHtml = searchMatch[1];
    cleanedToolContent = toolContent.replace(
      /<!-- SEARCH_SUGGESTIONS_HTML:[\s\S]*? -->/i,
      "",
    );
    debugLog(
  ...
  const systemPromptsText = systemPrompts
    ? combineSystemPrompts(systemPrompts)
    : STYLING_PROMPT; // Ensure styling if others disabled
...
function extractSearchSuggestions(responseText: string): {
  finalContent: string;
...
async function processQueryWithTools(
  ...
  if (
    rankingResult.rankedToolGroups.length > 0 &&
    ...
    try {
      ...
      if (responseData.searchSuggestionsHtml) {
        await ctx.runMutation(api.chat.updateMessageMetadata, {
          messageId,
          metadata: {
            searchSuggestionsHtml: responseData.searchSuggestionsHtml,
          },
        });
      }
      ...
      if (messageId) {
        try {
          ...
          console.error(
            "[getAIResponse] Error updating message with error state:",
            updateError,
          );
        }
      } else {
        ...
        try {
          ...
          console.error(
            "[getAIResponse] Error creating error message:",
            createError,
          );
          ...
          throw new ConvexError(
            "Failed to process request and failed to save error message.",
          );
        }
      }
      ...
      if (!messageId) {
        ...
        throw new ConvexError(
          "Critical error: No message ID available after failure.",
        );
      }
      return messageId;
    }
